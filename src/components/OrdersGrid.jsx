import { splitArrayToNewLine } from "@src/utils";
import { Loader } from "@components/Loader/Loader";

export const OrdersGrid = ({ isLoading, response, orderId, deleteOrder }) => (
  <div className="row">
    <div
              ...
              {Object.keys(order).map((orderElement, key) => (
                <div className="row" key={key}>
                  ...
                  <div className="col-md-6 text-start border d-flex align-items-center">
                    {typeof order[orderElement] === "object"
                      ? splitArrayToNewLine(order[orderElement])
                      : order[orderElement]}
                  </div>
                </div>
              ))}
